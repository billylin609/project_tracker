...
fn main() {
    ...
    let mut result = extract_data(&file_data);
    let score = updated_score(& mut result);
...
fn updated_score (data_info:& mut Vec<EachInput>) -> i64 {
    let mut score: i64 = 0;
    let mut type_score: i8;
    for item in data_info {
        type_score = match item.you {
            1 => -1,
            2 => 0,
            3 => 1,
            _ => panic!("value out of bound"),
        };
        type_score = type_score + item.opponent;
        if type_score == 0 {
            type_score = 3;
        } else if type_score == 4 {
            type_score = 1;
        }
        item.you = match item.you {
            1 => 0,
            2 => 3,
            3 => 6,
            _ => panic!("unknown caputer"),
        };
        println!("type_score: {} \nitem.you: {}", type_score, item.you);
        score = type_score as i64 + item.you as i64 + score;
    }
    return score;
}
